...
interface OfficeChairsSectionProps {
  products: Product[];
}

export function OfficeChairsSection({ products }: OfficeChairsSectionProps) {
  const [selectedType, setSelectedType] = useState(CHAIR_TYPES[0]);
  ...
  const scrollContainer = (direction: "left" | "right") => {
    const container = document.getElementById("office-chairs-carousel");
    if (container) {
  ...
  return (
    <section className="pt-5 pb-16 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
      <div className="max-w-[85rem] mx-auto px-4">
        ...
        {/* Latest Office Chairs Collections Section */}
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="flex-1">
              <h3 className="text-xl md:text-2xl font-heading font-bold text-slate-900 dark:text-white mb-3 tracking-tight">
                Latest Office Chair Collections
              </h3>
              <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed max-w-3xl">
                Enhance comfort and productivity with ergonomic office chairs. From mesh to executive styles, find the perfect seating solution for your workspace.
              </p>
            </div>
            <Link
              href="/products?subcategory=office-chairs"
